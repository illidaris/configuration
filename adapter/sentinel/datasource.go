...
func (s *NacosDataSource) ReadSource() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to read the nacos data source when initialization, err: %+v", err)
	}
...
func (s *NacosDataSource) listen(client config_client.IConfigClient) (err error) {
	...
	if err != nil {
		return fmt.Errorf("Failed to listen to the nacos data source, err: %+v", err)
	}
	return nil
}
...
func (s *NacosDataSource) Close() error {
	...
	if err != nil {
		return fmt.Errorf("Failed to cancel listen to the nacos data source, err: %+v", err)
	}
